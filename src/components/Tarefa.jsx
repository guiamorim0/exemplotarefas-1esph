...
const Tarefa = () => {
    ...
    setTarefas([...tarefas,novaTarefa]); // os ... significa spred ->  
    setCampo(''); // limpa a tela
    }
    ...
    return (
        <div className="max-w-md mx-auto mt-10 bg-indigo-500 rounded-2xl shadow-lg shadow-amber-200 border border-white"> 
            ...
            <form onSubmit={AdicionarTarefa} className="flex gap-2 mb-6">
                <input type="text" value={campo} onChange={(e)=>setCampo(e.target.value)} placeholder="Digite sua Tarefa" className="flex-1 px-4 py-2 border border-gray-700 rounded-2xl focus:outline-none focus:ring-1 focus:ring-amber-200 focus:border-transparent text-black placeholder:text-gray-700 "/>
            ...
            <ul className="space-y-3">
                {tarefas.map((tarefa)=>(
                    <li key={tarefa.id} className="flex items-center justify-between p-3 bg-indigo-900 border border-amber-300 rounded-2xl shadow-sm hover:bg-indigo-500 transition-colors">
                    <span>{tarefa.texto}</span>
                    <button onClick={()=>RemoverTarefa(tarefa.id)} className="bg-red-950 hover:bg-red-400 text-amber-50 font-medium px-5 py-2 rounded-2xl transition-colors cursor-pointer">Excluir</button>
                    </li>
                ))}
            </ul>
